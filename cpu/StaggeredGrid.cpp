...
void StaggeredGrid::ApplyGravity(double dt, int frame, int step) {
...
void StaggeredGrid::ProjectPressure(int frame, int step) {
  ...
  // we reasonably can.
  pressure_solver_.ProjectPressure(cell_labels_, neighbors_, u_, v_, w_, &p_, frame, step);
...
bool StaggeredGrid::CheckCollision(const Particle& p1, const Particle& p2) const {
  Eigen::Vector3d diff = p1.pos - p2.pos;
  double dist2 = diff.squaredNorm();
  
  // Collision threshold is twice the particle radius (assuming particles are spheres)
  double collision_threshold = 2.0 * 0.0025;  // particle radius is 0.0025
  return dist2 < collision_threshold * collision_threshold;
}

void StaggeredGrid::ResolveCollision(Particle* p1, Particle* p2) {
  Eigen::Vector3d diff = p1->pos - p2->pos;
  ...
  // Normal vector pointing from p2 to p1
  ...
  // Impulse calculation
  double j = -(1.0 + cor) * rel_vel.dot(normal) / 2.0;  // Assuming equal masses
  ...
  double overlap = collision_threshold - dist;
  if (overlap > 0) {
...
void StaggeredGrid::HandleParticleCollisions(std::vector<Particle>* particles) {
