...
class StaggeredGrid {
 public:
  ...
  const Array3D<double>& fu() const { return fu_; }
  const Array3D<double>& fv() const { return fv_; }
  const Array3D<double>& fw() const { return fw_; }
  const Array3D<MaterialType>& cell_labels() const { return cell_labels_; }
  const Array3D<unsigned short>& neighbors() const { return neighbors_; }
  ...
  // in this grid.
  void ApplyGravity(double dt, int frame, int step);
  ...
  void ProjectPressure(int frame, int step);
  ...
  // Handles collisions between all particles
